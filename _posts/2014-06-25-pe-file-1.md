...
- [PE文件格式总览][pe文件格式总览]
- [PE文件的段（PE File Sections）][pe文件的段（pe-file-sections）]
- [相对虚拟地址（Relative Virtual Address）][相对虚拟地址（relative-virtual-address）]
- [数据目录（data directory）][数据目录（data-directory）]
- [导入函数（importing functions）][导入函数（importing-functions）]
- [PE文件结构（PE File Structure）][pe文件结构（pe-file-structure）]
- [MS-DOS 头（MS-DOS Header）][ms-dos-头（ms-dos-header）]
- [IMAGE_NT_HEADERS 头][image_nt_headers-头]
- [段表（Section Table）][段表（section-table）]
