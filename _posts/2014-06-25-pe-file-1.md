...
原文地址：(http://msdn.microsoft.com/en-us/magazine/cc301805.aspx)
...
我在1997年MSJ的Under the Hood栏目中，对链接器的算法给出了一些介绍。
...
图2描述了每一个IMAGE_DIRECTORY_ENTRY_XXX值的指向。
...
图3显示了`IMAGE_FILE_HEADER`结构体中的字段，也包括针对每个字段的解释。
这个结构体也能在COFF OBJ文件的最开始找到。
图4列出了`IMAGE_FILE_XXX`常用的值。图5展示了`IMAGE_OPTINAL_HEADER`结构体中的字段们。
...
图6描述了`IMAGE_SECTION_HEADER`中的字段。
